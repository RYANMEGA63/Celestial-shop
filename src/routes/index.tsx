...
import { ArrowRight, Cpu, Gamepad2, Brain, Briefcase, Zap, Loader2 } from "lucide-react";
  ...
  const { data: dbProducts = [], isLoading } = useQuery<Product[]>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .order("price", { ascending: false }); // Sort by price descending (most expensive first)
      if (error) throw error;
      return data;
    },
  });

  // Extract the most expensive product from up to 6 different categories
  ...
  return (
    <main>
      ...
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative mx-auto grid max-w-[1400px] gap-8 px-3 py-10 sm:px-4 sm:py-16 md:grid-cols-2 md:py-24">
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Button
                asChild
                size="lg"
                className={`bg-success text-success-foreground hover:bg-success/90 transition-all duration-700 ${
                  ctaReady
                    ? "ring-4 ring-success/35 shadow-[0_0_35px_rgba(16,185,129,0.6)] scale-105"
                    : ""
                }`}
              >
          ...
          <div className="relative flex items-center justify-center">
            <RoboticAssembly onAssemblyComplete={() => setCtaReady(true)} />
          </div>
        </div>
      </section>
          ...
          {universes.map((u) => {
            ...
            return (
              <Link
                ...
                className={`group relative overflow-hidden rounded-xl border border-border bg-card p-6 transition-all hover:-translate-y-1 hover:border-primary hover:shadow-lg`}
              >
                ...
                <div className="relative">
                  ...
                  <div className="mt-6 inline-flex items-center gap-1 font-mono text-xs uppercase tracking-widest text-primary">
                    Voir les builds <ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-1" />
                  </div>
                </div>
              </Link>
            );
      ...
      <section className="border-t border-border bg-muted/30 py-16">
        <div className="mx-auto max-w-[1400px] px-3 sm:px-4">
          <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            <Link to="/boutique" className="font-mono text-xs uppercase tracking-widest text-muted-foreground hover:text-primary">
              Catalogue complet →
            </Link>
          </div>
      ...
      <ProductDetailSheet
        ...
        onAdd={(p) => toast.success(`${p.name} ajouté au panier`)}
      />
    </main>
  );
}
